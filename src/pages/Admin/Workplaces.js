...
const Workplaces = () => {
    ...
    const sidebarLink = (item, key, count) => {
        ...
        return item?.childNodes === 'true' ? (
            <Disclosure key={key}>
                {({open}) => {
                    if (open) getWorkplaceNode(key);
                    return (
                        <>
                                        ...
                                        <div className='flex flex-col items-start grow'>
                                            <div className='text-sm flex items-center justify-start text-left w-full'>
                                                ...
                                                >
                                                    ...
                                                    <span className='flex flex-wrap items-center'>{item?.name}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {item?.childNodes === 'true' && item?.children?.length > 0 ? (
                                <Disclosure.Panel className={classNames(`ml-${count + 1}`, 'text-gray-500')}>
                                    {item?.children?.map((item) => sidebarLink(item, item?.id, count))}
                                </Disclosure.Panel>
                            ) : ''}
                        </>
                    )
                                    ...
                                    <div className='flex flex-col items-start grow'>
                                        <div className='text-sm flex items-center justify-start text-left w-full'>
                                            ...
                                            >
                                                <img src={icons[item.icon]} alt='Значок' className='h-4 w-4'/>
                                                <span
                                                    className='flex flex-wrap items-center'>{item.brand} {item.model}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
