        ...
        <div className="px-4 py-5 sm:p-4">
          <div className="flex flex-col my-4">
            {sortFilter().length > 0 ? sortFilter()
                ...
                .map((item, key) =>
                  <div key={item.id} className={classNames("flex grow items-center px-2 py-3", key % 2 === 0 ? '' : 'bg-slate-100')}>
                    <Avatar size="10" name={item.fullname} shape="circle" classname='mr-5' />
                    <div className="flex flex-col items-start mr-7 w-72">
                      <p
                        className="font-medium text-sm text-slate-800 flex flex-wrap justify-start items-center text-left mb-1">{getHighlightedText(item?.fullname, query)}</p>
                      <p
                        className="text-sm text-slate-700 flex flex-wrap justify-start items-center text-left">{item?.profession}</p>
                    </div>
                    <div className="flex flex-col items-start">
                      <p
                        className="font-medium text-sm text-slate-800 flex justify-start items-center text-left mb-1">
                      ...
                      <p
                        className="text-sm text-slate-700 flex justify-start items-center text-left">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2">
