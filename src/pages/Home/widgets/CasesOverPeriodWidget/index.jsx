...
const CasesOverPeriodWidget = ({ rows, isLoading, error }) => (
    error === null ?
    <div className="rounded-lg relative group bg-white dark:bg-gray-900 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500 shadow">
        <Link to={isLoading === "true" ? "" : "/over-period"} className="w-full focus:outline-none min-w-0">
            <div className="flex items-center p-3">
                {isLoading === "true" ?
                    <>
                        <AvatarCircularWithPlaceholderInitials name="" size="14" color="red"
                                                               icon={
                    ...
                    :
                    <>
                        <AvatarCircularWithPlaceholderInitials name={rows?.length.toString()} size="14" color="red" shape="roundedLG" classname="flex-shrink-0" />
