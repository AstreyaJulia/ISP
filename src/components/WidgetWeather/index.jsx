          ...
          {currentError === null && weather?.weather ?
            <>
                <div className="flex flex-col gap-1 p-5">
                    ...
                    <p
                      className="font-bold text-2xl text-gray-100">{Math.ceil(weather?.main?.temp) > 0 ? `+ ${Math.ceil(weather?.main?.temp)}` : Math.ceil(weather?.main?.temp)}
                        <sup className='font-medium text-sm'>
                            <span><sup className='ml-1'>o</sup>C</span>
                        </sup>
                    </p>
                </div>

                <div className="flex flex-col items-end justify-end h-full">
                    <img title={weather?.weather[0]?.description} src={weatherStates[weather?.weather[0]?.id].icon}
