            ...
            <div className='flex items-center space-x-3'>
              {options.map((option) => (<RadioGroup.Option
                ...
                className={({
                              active,
                              checked,
                            }) => classNames(option.value.toString() === '2' ? 'ring-pink-500' : 'ring-cyan-500', active && checked ? 'ring ring-offset-1' : '', !active && checked ? 'ring-2' : '', '-m-0.5 relative p-0.5 rounded-full flex items-center justify-center cursor-pointer focus:outline-none')}
              >
                ...
                <span
                  aria-hidden='true'
                  className={classNames(option.value.toString() === '2' ? 'bg-pink-400' : 'bg-cyan-400', 'h-8 w-8 border border-black border-opacity-10 rounded-full flex items-center justify-center shadow-sm')}
                >{option.value.toString() === '2' ?
