    ...
    <Controller
      ...
      render={({ field, fieldState: { error } }) => (
        <div className={classNames('flex w-full', direction === 'row' ? 'justify-end' : 'flex-col', label ? 'gap-4' : '')}>
          ...
          {label ? (
            <label htmlFor={name} className={classNames('flex flex-col shrink-0 text-base font-medium text-gray-700 dark:text-gray-200', direction === 'row' ? 'text-right w-52' : 'w-full text-left ')} >
              {label}
            </label>
          ) : (
            ''
          ...
          <div className="relative rounded-md shadow-sm w-full">
            <textarea
              ...
              className={classNames(
                'block bg-gray-100 dark:bg-gray-800 w-full py-2 px-3 focus:outline-none text-base rounded-lg',
                error
