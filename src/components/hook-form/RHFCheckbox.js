              ...
              {options.map((option) => (
                <div key={`${name}-${option.value}`} className='flex items-center' {...other}>
                  <input {...field} onChange={(evt) => field.onChange(onSelected(evt, option))}
                         className={classNames(inputOptions.sizes[size].classNames.input, 'rounded', error ? 'focus:ring-red-500 text-red-600 border-red-400' : inputOptions.colors[color].classNames.input, 'border-gray-400 disabled:border-gray-200 disabled:checked:border-0')}
                         checked={field.value.includes(option.value)}
                         type='checkbox' id={`${name}-${option.value}`}
                         disabled={disabled}
                         name={`${name}`} value={option.value} />
                  <label className={inputOptions.sizes[size].classNames.label}
