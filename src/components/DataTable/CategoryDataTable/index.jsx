      ...
      <table className="w-full mt-2">
        ...
        <tr className=" rounded-md">
          {columns.slice(0, 2).map((column, key) => (
            <th key={key} className="pr-3 py-3 text-left bg-slate-100 dark:bg-slate-800 border-b border-slate-300 dark:border-slate-600">
              <div
        ...
        {sortFilter().length > 0 ? sortFilter()
          ...
          .map((row, key) => (
            <tr key={row + key}>
              {columns.slice(0, 2).map((column, key) => (
                <td key={column + key} className="border-b border-slate-300 dark:border-slate-600 py-2 px-3 text-sm text-slate-700 dark:text-slate-200">
                  {row[column]}
                </td>
              ))}
            </tr>
