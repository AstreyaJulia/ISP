...
import CasesOverPeriodDecisionListFile from './CasesOverPeriodDecisionListFile';
import { getUniqueArrayValuesByKey } from '../../../utils/getArrayValuesByKey';
import { getInitials } from '../../../utils/getInitials';
import Badge from "../../Badge";

const CasesOverPeriodDecision = ({ data, isLoading, error, all }) => {
  ...
  const getStatusSettings = (status, item) => {
    const statusSettings = {
      ...
      overperiod_decision: { title: 'Нарушение срока принятия', color: 'red' },
    };
    if (status) {
  ...
  return (
    <>
      ...
      <DataTableCore
        ...
        tableID="cases-over-period-вусшышщт"
        isLoading={isLoading}
        error={error}
        columns={columns}
        itemsContainerClassNames={isLoading === 'true' ? 'flex w-full' : 'grid grid-cols-2 gap-3'}
        initSortColumn={columns[0]}
        placeholder="Поиск по делам, принятым к производству свыше срока: номер дела / стороны / лица"
      ...
      >
        ...
        <PdfModal onModalClose={handlePDFModalClosed} open={modalPDFOpened} setOpen={setModalPDFOpened}>
          <PDFViewer width="100%" height="100%" style={{ border: 'none' }}>
            <CasesOverPeriodDecisionListFile list={rows} />
          </PDFViewer>
        </PdfModal>
      </DataTableCore>
      <CaseInfoModal open={modalOpened} setOpen={setModalOpened} onModalClose={handleModalClosed} item={selectedCase} caseStatus={{
        ...
        <div className="mt-7">
          <p className="text-gray-600 dark:text-gray-400 text-sm font-bold mb-2">Дата поступления дела:</p>
          <p className="text-gray-700 dark:text-gray-300 text-sm mb-5">{selectedCase.ENTRY_DATE}</p>
          <p className="text-gray-600 dark:text-gray-400 text-sm font-bold mb-2">Дата определения о (принято / отказано / без движения):</p>
          <p className="text-gray-700 dark:text-gray-300 text-sm mb-5">{selectedCase.DECISION_DATE}</p>
...
CasesOverPeriodDecision.propTypes = {
  /** Массив элементов  */
...
export default CasesOverPeriodDecision;
