...
const Processed = ({ data, isLoading, all }) => {
  ...
  const [selectedFilter, setSelectedFilter] = useState({ JUDGE: "All", CASE_STATUS: "All" });
  ...
  useEffect(() => {
    setRows(data);
    setJudgesList(getUniqueArrayValuesByKey(data ?? [], "JUDGE").sort((a, b) => a.localeCompare(b)));
      ...
      <div className="flex grow items-center">
        <div className="flex items-center justify-between w-full">
          <div className="flex flex-col items-start">
            ...
            <span className="text-xs text-indigo-700 dark:text-indigo-300 mb-2">{getInitials(item?.JUDGE)}</span>
            <p
              className="text-sm text-slate-700 dark:text-slate-300 flex flex-wrap line-clamp-1 justify-start items-center text-left">
              {getHighlightedText(item?.PARTS_NAMES, query)}
            </p>
          </div>
  ...
  const filter = (rows, query, columns) => rows?.filter((row) =>
    columns.slice(3, 7).filter((item) => item !== "JUDGE" && item !== "JUDGE_ID").some(
      ...
      <div className="p-3 bg-slate-100 rounded-md mt-3 mx-4 flex items-center gap-2">
        {all === "true" ? <div className="flex items-center ml-3 justify-start">
          ...
          <select
            id="JUDGE"
            name="JUDGE"
            defaultValue={selectedFilter.JUDGE}
