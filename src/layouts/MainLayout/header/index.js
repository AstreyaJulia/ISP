...
const Header = ({ setMenuVisibility }) => {
  const searchTypes = {
    ...
    inbox: {
      selectText: 'Входящая почта',
      searchText: 'Поиск по входящей корреспонденции по номеру / Ф.И.О. / содержанию',
    },
    outbox: {
      selectText: 'Исходящая почта',
      searchText: 'Поиск исходящей корреспондеции по номеру / Ф.И.О. / содержанию',
    },
  };
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      ...
      if (searchQuery.length < 3 && parseInt(searchQuery, 10).toString().length < 1) {
        ...
        if (searchResultsShow === false && searchQuery.length > 0) changeSearchResultsShow(true);
      }
    }, 2000);
  ...
  const ThemeToggler = () => {
      ...
      return (
        ...
        >
          ...
          >
            ...
            <path
              d='m2.75 13.25.5-.5m9.5 0 .5.5m-.5-10 .5-.5m-10 .5-.5-.5m-.50 5.25h-1m13.5 0h-1m-5.75 5.75v1m0-13.5v1' />
          </svg>
        </button>
      );
    }
    return (
      ...
      >
        ...
        >
          <path
            d='m1.75 8c0 3.45 2.8 6.25 6.25 6.25 3.41-.0027 6.25-3 6.25-6-1 .5-4 1.5-6-.5s-1-5-.5-6c-3 0-6 2.84-6 6.25z' />
        </svg>
      </button>
    );
  };
